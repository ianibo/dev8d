...
class Person(db.Model):
    ...
    def get_absolute_url(self):
        return ('cook.views.show_person', (), {'key': self.key()})
...
class File(db.Model):
    ...
    @permalink
    def get_absolute_url(self):
        return ('cook.views.download_file', (), {'key': self.key(),
                                                  'name': self.name})
...
class Resource(db.Model):
    """A resource selected by a user."""
    owner = db.ReferenceProperty(Person, required=True, collection_name='resources')
    name = db.StringProperty(required=True)
